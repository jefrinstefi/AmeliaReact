...
const BusinessIntents = ({ data }) => {
  ...
  useEffect(() => { 
    // if (data.business_intent) {
    //   const intentsArray = Object.entries(data.business_intent).map(
    //     ([key, value]) => ({
    ...
    //     })
    //   );
        if (Array.isArray(data.business_intent)) {
    const intentsArray = data.business_intent.map((item) => ({
     name: item.intent.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase()), // Format nicely
      value: item.intent_count,
      color: barColors[Math.floor(Math.random() * barColors.length)],
    }));
...
export default BusinessIntents;